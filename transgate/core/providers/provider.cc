...
int Provider::writeHead(int ver_major, int ver_minor, HttpStatusCode code) {
  int ret = write_loop_->swrite("HTTP/%d.%d %d %s\r\n", ver_major, ver_minor, code, httpCodeDescription(code));
  ret += write_loop_->swrite("Server: Transgate %s\r\n", detail::version());
  return ret;
}

int Provider::writeItem(const char *key, const char *value) {
  return write_loop_->swrite("%s: %s\r\n", key, value);
}

int Provider::writeItemDate() {
  time_t t = time(nullptr);
  char tmbuf[64];
  strftime(tmbuf, 63, "Date: %a, %d %b %Y %H:%M:%S GMT\r\n", gmtime(&t));
  return write_loop_->write(tmbuf);
}

int Provider::writeItemConnection(bool keep_alive) {
  return writeItem("Connection", keep_alive ? "keep-alive" : "close");
}

int Provider::writeCRLF() {
  return write_loop_->write("\r\n", 2);
}
...
int Provider::provideError() {
  ...
  int length = sprintf(buffer.get(), templat, static_cast<int>(code), httpCodeDescription(code), detail::version());
  int ret = regularProvide(request_->isKeepalive(), length - 2, defaultMIME());
  ret += writeCRLF();
  ret += write_loop_->swrite(buffer.get(), length);
...
int Provider::regularProvide(bool keep_alive, long content_length, const char *mime) {
  ...
  ret += writeItem("Content-Type", mime);
...
const char *Provider::adaptMIME() const {
  ...
  return ext_start == request_->uri().readptr() ?
         defaultMIME() :
         mimeTable(ext_start, static_cast<size_t>(std::distance(ext_start, ext_end)));
}

const char *Provider::mimeTable(const char *ext, size_t n) const {
#define ie(str, mime) if (strncasecmp(ext, str, n) == 0) return mime
  ie (".tif", "image/tiff");
